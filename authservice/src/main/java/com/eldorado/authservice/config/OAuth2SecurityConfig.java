...
public class OAuth2SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests()
				.mvcMatchers("/oauth/**", "/hello", "api/auth/getaccesstoken", "api/auth/getrefreshtoken").permitAll()
				.antMatchers("/swagger-ui/**").permitAll()
				.antMatchers("/oauth/token", "/api/oauth/token", "/gettoken/**").permitAll()
				.antMatchers("/v3/api-docs/**").permitAll().anyRequest().authenticated().and().csrf().disable()
				.httpBasic();
	}
